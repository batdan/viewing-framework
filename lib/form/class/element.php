	...
	*/
	...
	private function chpInputButton()
	{
		...
		if (in_array($this->_type, $type_button)) {
			...
			// Boutons radio verticaux ou horizontaux
			if (isset($this->_options['vertical']) && $this->_options['vertical'] === true) {

				if (isset($this->_options['justified']) && $this->_options['justified'] === true) {
					$radioContainer->setAttribute('class', 'btn-group-vertical-justified');
				} else {
				...
				if (isset($this->_options['justified']) && $this->_options['justified'] === true) {
					$radioContainer->setAttribute('class', 'btn-group-justified');
				} else {
			...
			if (empty($this->_options['dataList'])) {
				...
				foreach ($this->_options['dataList'] as $k=>$v) {
					...
					// Insertion du input dans la balise label
					$label->appendChild($input);

					// Insertion de code html ou de texte dans les boutons radio
					if (strstr($v, '<') === false) {
		                $text = $this->_dom->createTextNode( ' ' . $v );
		                $label->appendChild($text);
		            } else {
		                $label->appendChild( $this->addHtml($v) );
		            }
