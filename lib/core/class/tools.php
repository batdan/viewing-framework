...
class tools
{
    ...
    public static function diffusionProg($dateDeb, $dateFin, $type='lin', $objectif=100, $coeff=1)
    {
        ...
        $nbJoursToday = $interval2->format('%a');

        if ($interval->format('%R') == '-') {
            if (class_exists('\FB')) {
                \FB::log('la date de fin inférieure à celle de début');
                return;
            } else {
                return 'la date de fin inférieure à celle de début';
            }
        }

        $nbJours = intval($interval->format('%a'));     // Nombre de jours atteint depuis de début de la diffusion
        $res     = array();
